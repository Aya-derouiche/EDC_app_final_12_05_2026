...
router.post("/upload", requireRole("admin", "comptable", "client"), upload.single("file"), async (req, res, next) => {
  try { const doc = await documentService.uploadAndProcess({ tenantId: req.tenantId, userId: req.user.id, file: req.file, documentType: req.body.documentType }); res.status(201).json(doc); } catch (e) { next(e); }
});
